...
kubectl -n consul create secret generic vault \
    --from-file=certs/ca.pem \
...
kubectl -n consul create configmap vault --from-file=vault/config.json


echo "Creating the Vault Service..."

kubectl -n consul create -f vault/service.yaml


echo "Creating the Vault Deployment..."

kubectl -n consul apply -f vault/deployment.yaml


echo "All done! Forwarding port 8200..."

POD=$(kubectl -n consul get pods -o=name | grep vault | sed "s/^.\{4\}//")

while true; do
  STATUS=$(kubectl -n consul get pods ${POD} -o jsonpath="{.status.phase}")
...
kubectl -n consul port-forward $POD 8200:8200
